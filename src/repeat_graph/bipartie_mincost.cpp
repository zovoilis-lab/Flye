// Implementation was taken from https://github.com/t3nsor/codebook
//
...
namespace
{
	...
	double _MinCostMatching(const VVD &cost, VI &Lmate, VI &Rmate) {
	  int n = int(cost.size());
...
std::vector<int> bipartieMincost(const BipartieTable& cost)
{
	VI leftMate;
	VI rightMate;
	_MinCostMatching(cost, leftMate, rightMate);
	return leftMate;
}
