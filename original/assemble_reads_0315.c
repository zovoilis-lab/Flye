...
//test if two reads overlap or not, the first shared vertex (i1,j1), and the last shared vertex (i2,j2),  
					   ...
					   int min_coverage, int number_of_reads)
{
	...
	for (int read_id = 0; read_id < number_of_reads ; read_id++)
	{
		...
		else
		{
			if(linked[read_id] == 1)
			{
				for (int i = start_align_cur[read_id] / 100 + MAX_JUMP/100; i < end_align_cur[read_id]/100 - MAX_JUMP/100; i++)
				{
					chimeric[i]++;
				}
	...
	for (int i = (MAX_OVERHANG+MAX_JUMP) / 100; i < Reads[current_read].len/100 - (MAX_OVERHANG+MAX_JUMP)/100; i++)
	{
			...
			else
			{
				if(linked[read_id] == 1)
				{
					if (Reads[current_read].overlap_read_count < 200)
					{
						...
						Reads[current_read].overlap_start_pos1[Reads[current_read].overlap_read_count] =  start_align_cur[read_id];
						Reads[current_read].overlap_end_pos1[Reads[current_read].overlap_read_count] =  end_align_cur[read_id];
						Reads[current_read].overlap_start_pos2[Reads[current_read].overlap_read_count] =  start_align_other[read_id];
						Reads[current_read].overlap_start_pos2[Reads[current_read].overlap_read_count] =  end_align_other[read_id];
						...
						//{
							...
							//		temp_index, Reads[current_read].len, Reads[current_read].overlap_start_pos1[temp_index], 
							//		Reads[current_read].overlap_end_pos1[temp_index], read_id, Reads[read_id].len, 
							//		Reads[current_read].overlap_start_pos2[temp_index], 
							//		Reads[current_read].overlap_end_pos2[temp_index]);
						//}
						if (read_id % 2 == 0)
						{
							Reads[current_read + 1].overlap_read[Reads[current_read + 1].overlap_read_count] = read_id + 1;
							Reads[current_read + 1].overlap_start_pos1[Reads[current_read + 1].overlap_read_count] = 
								Reads[current_read].len - end_align_cur[read_id];
							Reads[current_read + 1].overlap_end_pos1[Reads[current_read + 1].overlap_read_count] = 
								Reads[current_read].len - start_align_cur[read_id];
							Reads[current_read + 1].overlap_start_pos2[Reads[current_read + 1].overlap_read_count] = 
								Reads[read_id + 1].len - end_align_other[read_id];
							Reads[current_read + 1].overlap_start_pos2[Reads[current_read + 1].overlap_read_count] = 
								Reads[read_id + 1].len - start_align_other[read_id];
