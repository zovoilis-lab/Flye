...
void OverlapDetector::parallelWorker()
{
	...
	while (true)
	{
		...
		auto detectedOverlaps = this->getSelfOverlaps(readId);
		_fetchMutex.lock();
						  ...
						  int32_t extPrev, int32_t extNext) const
{
	...
	if (0 < curNext - curPrev && curNext - curPrev < JUMP &&
		0 < extNext - extPrev && extNext - extPrev < JUMP)
	{
		if (abs((curNext - curPrev) - (extNext - extPrev)) 
			< JUMP / Constants::closeJumpRate)
		{
			return J_CLOSE;
		}
		if (abs((curNext - curPrev) - (extNext - extPrev)) 
			< JUMP / Constants::farJumpRate)
		{
			return J_FAR;
		}
	}
...
std::vector<OverlapRange> 
OverlapDetector::getSelfOverlaps(FastaRecord::Id currentReadId) const
{
	std::unordered_map<FastaRecord::Id, 
					   std::vector<OverlapRange>> activePaths;
	std::set<size_t> eraseMarks;
